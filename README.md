...
- **AI 聊天** — 支持双模型切换（GLM 高质量 / Qwen 快速），结合用户课表/考试/成绩数据智能对话；请求经自建安全代理转发，密钥不随 APK 分发
...
- 自建 LLM 代理服务（杭州节点）：模型密钥仅存服务端，设备级限流 + 模型白名单 + 用量封顶
...
### v2.5.7
- **更新说明支持 Markdown 渲染**
  - 更新弹窗正确渲染标题、项目列表、粗体、斜体、行内代码和链接，不再原样显示 `##`、`-` 等 Markdown 标记。
  - 自动移除与弹窗重复的版本标题，并将安装包大小作为次要信息展示。
  - 首页更新卡片使用清理后的内容摘要，避免截断后残留 Markdown 语法。
- **文档与发布信息同步**
  - README 当前版本、下载地址和更新日志同步至 v2.5.7。

### v2.5.6
- **自动重登录与账号会话加固**
  - 手动登录、自动重登录和密码修改统一串行化，验证码与对应 token、登录地址和 ViewState 独立绑定。
  - 切换账号、退出登录或更改教务地址后，旧重登录结果不会覆盖当前账号，并会清理旧 Cookie。
  - 登录相关网络请求支持协程取消；服务器繁忙不再被误判为会话过期，登录成功必须验证教务主页特征。
- **成绩定时检查增强**
  - 后台成绩检查迁移到 WorkManager；会话过期时可自动重登录，并增加网络约束、超时和重试保护。
  - 修复成绩首次基线、解析字段和新增成绩通知相关问题。
- **课表缓存与二维课表修复**
  - 修复寒暑假期间二维课表反复清除缓存、每次进入都重新联网的问题。
  - 普通进入优先使用缓存，新学期课表按周期探测，手动刷新仍会立即联网。
  - 修复二维课表刷新无反馈，增加刷新状态、结果提示及防重复点击。
  - 优化刷新提示配色、课程去重、周起始日期和课程排序。
- **稳定性改进**
  - 改进课程提醒、首页、桌宠、番茄钟及反馈服务稳定性。
  - 新增会话响应、课程身份和成绩解析单元测试。
